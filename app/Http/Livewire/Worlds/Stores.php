...
namespace App\Http\Livewire\Worlds;
...
class Stores extends Component
{
    ...
    {
        $stores = $this->getTransactionsUsingTitles($this->worldId);
        $contentRows = [];
        $specialClasses = 'show active';
        $active = 'active';
        foreach($stores as $name => $store) {
            $contentRows[] = (object) ['gridName' => $name, 'active' => $active, 'SpecialClass' => $specialClasses, 'JSID' => $store->jsid, 'Owner' => $store->owner,
            'NavigationRows' => $this->navigationRows($store), 'GPS' => $store->GPS, 'GoodTypeRows' => $this->goodTypeRows($store)];
            $specialClasses = '';
            $active = '';
        }
        return $contentRows;
    ...
    public function goodTypeRows($store) : array
    {
        ...
        $specialClasses = 'show active';
        foreach($this->goodTypes as $goodType) {
            $goodTypeRows[] = (object) ['SpecialClasses' =>$specialClasses, 'JSID' => $store->jsid, 'GoodType' => $goodType, 'Goods' => $store->goods];
            $specialClasses = 'hide';
        }
