...
class SqliteWalEnable extends Command
{
    ...
    protected $signature = 'sqlite:wal-enable
                            {connection=storage : The connection to enable WAL journal}';
    ...
    {
        $dbsToEnableWal = ['storage', 'transactions', 'trends'];
        foreach($dbsToEnableWal as $dbToWal) {
            $this->setWalJournalMode($db = $this->getDatabase($manager, $connection = $dbToWal));
